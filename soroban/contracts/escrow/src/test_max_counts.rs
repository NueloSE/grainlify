#![cfg(test)]
//! Stress tests for maximum bounty counts.
//!
//! Priority: verify that `env.storage().persistent()` handles a high volume
//! of `Escrow(bounty_id)` and `EscrowIndex` entries without key collisions.
//!
//! Counts and sampling checks use `get_escrow(id)` directly (the only count
//! mechanism currently exposed). Full search/count is tracked in Issue #789.
...
/// Lock 60 bounties and verify that each `Escrow(id)` storage entry is
/// independently retrievable with the correct data.
///
/// This is the primary check that `env.storage().persistent()` does not
/// produce key collisions across a high volume of `Escrow(bounty_id)` keys.
/// Each bounty's amount = id * 100, so any collision or overwrite is visible.
#[test]
fn test_max_bounties_no_storage_key_collision() {
    let env = Env::default();
    // 60 bounties × 100 tokens each = 6_000 (below Unverified tier limit per tx)
...
/// Lock 60 bounties where each bounty's amount equals `id * 10`. Spot-check
/// 5 specific IDs to confirm the right data is stored under each key.
///
/// Verifies the `EscrowIndex` ordering hasn't shuffled or overwritten entries.
#[test]
...
/// Verifies that `env.storage().persistent()` rejects duplicate keys and
...
/// deadline. Verify that release and refund operations on one entry do not
/// corrupt adjacent entries in `env.storage().persistent()`.
#[test]
fn test_max_bounties_lifecycle_no_cross_entry_corruption() {
    let env = Env::default();
    // 60 × 100 = 6_000 initial balance
    ...
    // Cross-check: released entries are still Released (not corrupted)
    for id in 1..=30u64 {
        assert_eq!(client.get_escrow(&id).status, EscrowStatus::Released);
    }
    // Cross-check: refunded entries are still Refunded
    for id in 31..=60u64 {
